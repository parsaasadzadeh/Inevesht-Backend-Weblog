...
exports.handleLogin = async (req, res, next) => {
    // گرفتن rememberMe از بادی ریکوئست
    ...
    try {
        ...
        if (isEqual) {
            // تعیین زمان انقضای توکن بر اساس مرا به خاطر بسپار
            // اگر تیک زده بود 30 روز، اگر نه 1 ساعت
            ...
            const token = jwt.sign(
                ...
                { expiresIn: tokenExpiration } // اضافه کردن زمان انقضا به JWT
            );
...
exports.handleForgetPassword = async (req, res, next) => {
    ...
    try {
        ...
        const resetLink = `https://backin-inev.runflare.run/users/reset-password/${token}`;
